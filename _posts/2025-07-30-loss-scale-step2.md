...
2. num_mircobatches = global_batch_size/(micro_batch_size*DP), num_mircobatches 与loss scale 最好是成倍数关系，比如num_mircobactes 为128时，loss scale 为256和512，*PUloss曲线更稳定。
...
$loss^{\prime}$为使用当前micro batch输入计算得到的损失值。为了减少梯度求平均的计算量，会先将$loss^{\prime}$除以micro batches数（这样累加后的梯度就无需再逐元素除以累加步数），然后再乘上loss_scale以调整梯度值到fp16所表示范围内。

1. loss_scale的作用是将梯度拉回到FP16所能表示的范围内，一般梯度值总是很小，故loss_scale主要是用于放大梯度值。因此，loss_scale取值必须要大于num_micro_batches，否则会起到反作用，即将计算得到的梯度值进行了缩小，这是必须避免的。
...
![](https://raw.githubusercontent.com/sherry-1001/sherry-1001.github.io/master/_images/adam_loss.png "")
2. adam+ transformer local， *PU和gpu loss 曲线收敛不一致 （排除transformer_engine 影响）

![](https://raw.githubusercontent.com/sherry-1001/sherry-1001.github.io/master/_images/adam_transform_local.png "")
3. `adam+ transformer local+ 删除掉clip grad` 和 ` 删除掉clip grad+删除掉loss/num_mircobatches` *PU loss 对比

![](https://raw.githubusercontent.com/sherry-1001/sherry-1001.github.io/master/_images/adam_transform_local_rm_clip.png "")
4. `adam+ transformer local+ 删除掉clip grad + 删掉loss/num_mircobatches`， *PU和gpu loss对比。（完全重合）

![](https://raw.githubusercontent.com/sherry-1001/sherry-1001.github.io/master/_images/rm_loss:num_micro.png "")
5. `adam+ transformer local+loss/num_mircobatches, 将fc全部改为fp32运算（输入全部为fp32）`，*PU和gpuloss 完全重合，可与2（下右图）的结果做对比

![](https://raw.githubusercontent.com/sherry-1001/sherry-1001.github.io/master/_images/fp32.png "")
![](https://raw.githubusercontent.com/sherry-1001/sherry-1001.github.io/master/_images/loss_miss.png "")